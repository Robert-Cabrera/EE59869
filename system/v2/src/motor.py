...
def init_motors():
    ...
    debug_print("[Motor] Initialized motors on pins L={} R={}".format(MOTOR_L, MOTOR_R))
...
def pulse_direction(direction):
    ...
    if direction == "LEFT":
        debug_print("[Motor] Pulsing LEFT motor")
        pulse(MOTOR_L)
    elif direction == "RIGHT":
        debug_print("[Motor] Pulsing RIGHT motor")
        pulse(MOTOR_R)
...
def cleanup_motors():
    ...
    if motors_initialized:
        try:
            ...
            motors_initialized = False
            debug_print("[Motor] Cleaned up motors")
        except Exception as e:
            motors_initialized = False
            debug_print(f"[Motor] Error during cleanup: {e}")
