...
def _initialize_clip():
    ...
    if not _initialized:
        try:
            ...
            debug_print(f"[AI][ENV] Error initializing CLIP: {e}")
            return False
    return True
...
def process_env(env_model, frame):
    ...
    if env_model is None:
        debug_print("[AI][ENV] Model not loaded")
        return "unknown"
    
    if frame is None:
        debug_print("[AI][ENV] Error during inference: Received None frame")
        return "unknown"
    
    if not _initialize_clip():
        debug_print("[AI][ENV] CLIP failed to initialize")
        return "unknown"
    ...
    if len(_frame_buffer) >= _buffer_size:
        try:
            ...
            debug_print(f"[AI][ENV] Prediction refreshed: {_last_voted_result}...")
            ...
            debug_print(f"[AI][ENV] Error during inference: {e}")
            return "unknown"
    
    # Buffer not full yet, return last cached result
    # debug_print(f"[AI][ENV] Buffer {len(_frame_buffer)}/{_buffer_size} - returning {_last_voted_result}")
    return _last_voted_result
