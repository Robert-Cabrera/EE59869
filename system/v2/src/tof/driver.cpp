...
int main() {
    ...
    while (true) {
        ...
        uint16_t idR = sensorR->read16Reg16(0x30, 0x010F);
        uint16_t idC = sensorC->read16Reg16(0x32, 0x010F);
        uint16_t idL = sensorL->read16Reg16(0x34, 0x010F);

        printf("IDs -> R:0x%02X C:0x%02X L:0x%02X | Dist -> R:%u C:%u L:%u   \r", 
                        idR, idC, idL, dR, dC, dL);
                fflush(stdout);
